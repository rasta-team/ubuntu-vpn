...
function add-user() {
	...
	mv -f /usr/local/etc/xray/vless_tmp.json /usr/local/etc/xray/vless.json
	cat /usr/local/etc/xray/trojan.json | jq '.inbounds[0].settings.clients += [{"password": "'${uuid}'","email": "'${email}'"}]' > /usr/local/etc/xray/trojan_tmp.json
	mv -f /usr/local/etc/xray/trojan_tmp.json /usr/local/etc/xray/trojan.json
...
function delete-user() {
	...
	mv -f /usr/local/etc/xray/vless_tmp.json /usr/local/etc/xray/vless.json
	cat /usr/local/etc/xray/trojan.json | jq 'del(.inbounds[0].settings.clients[] | select(.id == "'${uuid}'"))' > /usr/local/etc/xray/trojan_tmp.json
	mv -f /usr/local/etc/xray/trojan_tmp.json /usr/local/etc/xray/trojan.json
...
function user-monitor() {
	...
	for user in "${data[@]}"; do
		touch /tmp/ipxray.txt
		for ip in "${data3[@]}"; do
			total=$(cat /var/log/xray/access-trojan.log | grep -w ${user}@${domain} | awk '{print $3}' | cut -d: -f1 | grep -w $ip | sort | uniq)
			if [[ "$total" == "$ip" ]]; then
				echo -e "$total" >> /tmp/ipxray.txt
				n=$((n+1))
			fi
...
function show-config() {
	...
	uuid=$(cat /metavpn/xray/xray-clients.txt | grep -w "$user" | awk '{print $2}')
	domain=$(cat /usr/local/etc/xray/domain)
	path=$(cat /usr/local/etc/xray/path)
	...
	echo -e "TLS\t\t: tls"
	echo -e ""
	echo -e "Link : vless://$uuid@$domain:443?path=%2F${path}%2F&security=tls&encryption=none&host=${bug}&type=ws&sni=${bug}#XRAY_WS-$user"
	echo -e ""
	echo -e "${B}====================${N}"
	echo -e "  • Scan QR Code •  "
	echo -e "${B}====================${N}"
	qrencode -t ansiutf8 -l L "vless://$uuid@$domain:443?path=%2F${path}%2F&security=tls&encryption=none&host=${bug}&type=ws&sni=${bug}#XRAY_WS-$user"
	echo -e ""
	echo -e "${B}======================${N}"
	echo -e "   • VLESS + TCP •   "
	echo -e "${B}======================${N}"
	...
	echo -e "TLS\t\t: tls"
	echo -e ""
	echo -e "Link : vless://$uuid@$domain:443?security=tls&encryption=none&headerType=none&type=tcp&sni=${bug}#XRAY_TCP-$user"
	echo -e ""
	echo -e "${B}====================${N}"
	echo -e "  • Scan QR Code •  "
	echo -e "${B}====================${N}"
	qrencode -t ansiutf8 -l L "vless://$uuid@$domain:443?security=tls&encryption=none&headerType=none&type=tcp&sni=${bug}#XRAY_TCP-$user"
		echo -e ""
	echo -e "${B}======================${N}"
	echo -e "   • TROJAN + TLS •   "
	echo -e "${B}======================${N}"
	...
	echo -e "TLS\t\t: tls"
	echo -e ""
	echo -e "Link : trojan://$uuid@$domain:443?security=tls&encryption=none&headerType=none&type=tcp&sni=${bug}#XRAY_TROJAN-$user"
	echo -e ""
	echo -e "${B}====================${N}"
	echo -e "  • Scan QR Code •  "
	echo -e "${B}====================${N}"
	qrencode -t ansiutf8 -l L "trojan://$uuid@$domain:443?security=tls&encryption=none&headerType=none&type=tcp&sni=${bug}#XRAY_TROJAN-$user"
	echo -e ""
	else 
	bug="$domain"
	echo -e ""
