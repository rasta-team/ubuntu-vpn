...
wget -O /etc/stunnel/stunnel.conf "${repo}files/vless.conf" > /dev/null 2>&1
...
# Install Xray
xray_version="$(curl -s https://api.github.com/repos/XTLS/Xray-core/releases | grep tag_name | sed -E 's/.*"v(.*)".*/\1/' | head -n 1)"
xray_link="https://github.com/XTLS/Xray-core/releases/download/v$xray_version/xray-linux-64.zip"
...
echo $path > /usr/local/etc/xray/path
wget -O /usr/local/etc/xray/vless.json "${repo}files/xray/xray_vless.json" > /dev/null 2>&1
wget -O /usr/local/etc/xray/trojan.json "${repo}files/xray/xray_trojan.json" > /dev/null 2>&1
sed -i 's+xx+/${path}/+g' /usr/local/etc/xray/vless.json
...
wget -O /etc/nginx/conf.d/${domain}.conf "${repo}files/xray/nginx.conf" > /dev/null 2>&1
...
bash acme.sh --installcert -d $domain --fullchainpath /metavpn/xray/xray.crt --keypath /metavpn/xray/xray.key
iptables -I INPUT -m state --state NEW -m tcp -p tcp --dport 440 -j ACCEPT
iptables -I INPUT -m state --state NEW -m udp -p udp --dport 440 -j ACCEPT
...
iptables -I INPUT -m state --state NEW -m udp -p udp --dport 465 -j ACCEPT
iptables-save > /metavpn/iptables.rules
iptables-restore -t < /metavpn/iptables.rules
...
systemctl enable xray@vless > /dev/null 2>&1
systemctl enable xray@trojan > /dev/null 2>&1
check_run nginx
check_run xray@vless
check_run xray@trojan
(crontab -l;echo "0 * * * * echo '# Xray-VLESS access log (Script by Meta VPN)' > /var/log/xray/access-vless.log") | crontab -
(crontab -l;echo "0 * * * * echo '# Xray-TROJAN access log (Script by Meta VPN)' > /var/log/xray/access-trojan.log") | crontab -
...
wget -O /usr/bin/vless-script "${repo}files/menu/vless-script.sh" > /dev/null 2>&1
...
chmod +x /usr/bin/{menu,ssh-vpn-script,vless-script,wireguard-script,check-script,nench-script,stream-script}
